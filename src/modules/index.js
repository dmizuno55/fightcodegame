...
(function() {
  ...
  (function(calculator) {
    calculator.calculateAngle = function(basePos, targetPos) {
      var normalizedPoint = {
    ...
    calculator.calculateCannonAngle = function(basePos, targetPos) {
      var normalizedPoint = {
        x: targetPos.x - basePos.x,
        y: -(targetPos.y - basePos.y)
      };
    ...
    calculator.deltaAngle = function(baseDegrees, targetDegrees) {
  ...
  (function(command) {
    ...
    command.turnToDest = function(robot, dest) {
      ...
      log('dest', dest);
      var degrees = calculator.calculateAngle(mPos, dest);
      command.turnTo(robot, degrees);
    };

    command.turnTo = function(robot, degrees) {
      var log = utils.logger('command.turnTo', robot);
      degrees = calculator.deltaAngle(robot.angle, degrees);
    ...
    command.turnCannonTo = function(robot, degrees) {
      ...
      degrees = calculator.deltaAngle(robot.cannonAbsoluteAngle, degrees);
